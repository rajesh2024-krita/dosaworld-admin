          ...
          return (
            ...
            >
              ...
              <div className={cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
                {nestLabel ? tooltipLabel : <span className="text-muted-foreground">{itemConfig?.label || item.name}</span>}
      ...
      {payload.map((item) => {
        ...
        return (
          <div key={item.value} className="flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground">
            {itemConfig?.icon && !hideIcon ? <itemConfig.icon /> : <div className="h-2 w-2 shrink-0 rounded-[2px]" style={{ backgroundColor: item.color || "var(--primary)" }} />}
            {itemConfig?.label}
          </div>
        )
      })}
