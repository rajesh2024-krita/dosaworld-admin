...
const API_URL = "https://dosaworld-backend-xypt.onrender.com/api/billings"; // backend base URL
...
export default function EODBilling() {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            setData([{ ...form, id: Date.now() }, ...data]); // update UI
            setOpen(false);
            setForm({
                date: new Date().toISOString().split("T")[0],
                card: "",
  ...
  // If it's a full ISO datetime (YYYY-MM-DDTHH:mm:ssZ)
  ...
  return value;
                    ...
                    <table className="min-w-full text-left text-xs sm:text-sm">
                        <thead className="bg-muted-foreground/10 uppercase">
                            <tr>
                                ...
                                <th className="p-2 border-b">Total (Cash and Card)</th>
                            </tr>
                        </thead>
                        <tbody>
    ...
    const renderCharts = (rows: any[]) => {
        if (rows.length === 0) {
            return <div className="p-8 text-center text-muted-foreground">No data available for charts</div>;
        }
                ...
                <div className="flex items-center justify-between gap-4 w-full">
                    ...
                    <div className="flex items-center justify-between gap-2">
                        ...
                        {(filter.from || filter.to) && (
                            <Button variant="ghost" size="icon" onClick={() => setFilter({ from: "", to: "" })}>
                                <X className="h-4 w-4" />
                            </Button>
                        )}
                    ...
                    <form onSubmit={handleSubmit} className="grid gap-4 py-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-sm font-medium">Date</label>
                                <Input name="date" type="date" value={form.date} onChange={handleChange} readOnly />
                            </div>
                            <div><label className="text-sm font-medium">Handled By</label>
                                <Input name="handledBy" placeholder="Name" value={form.handledBy} onChange={handleChange} />
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-sm font-medium">Card (€)</label>
                                <Input name="card" type="number" step="0.01" value={form.card} onChange={handleChange} />
                            </div>
                            <div><label className="text-sm font-medium">Cash (€)</label>
                                <Input name="cash" type="number" step="0.01" value={form.cash} onChange={handleChange} />
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-sm font-medium">Trinkgeld (€)</label>
                                <Input name="trinkgeld" type="number" step="0.01" value={form.trinkgeld} onChange={handleChange} />
                            </div>
                            <div><label className="text-sm font-medium">Trinkgeld Bar (€)</label>
                                <Input name="trinkgeldBar" type="number" step="0.01" value={form.trinkgeldBar} onChange={handleChange} />
                            </div>
                        </div>
                        <div><label className="text-sm font-medium">Paid (Expenses) (€)</label>
                            <Input name="paid" type="number" step="0.01" value={form.paid} onChange={handleChange} />
                        </div>
