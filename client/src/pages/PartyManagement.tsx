...
// const Logo = "https://dosaworld.kritatechnosolutions.com/assets/logo-H7Hfdi3N.png"
...
const fetchLogoBytes = async (url: string) => {
  try {
    // If it's a local import, convert to data URL
...
// ===== Embed Image Safely =====
const embedImageSafely = async (pdfDoc: PDFDocument, logoBytes: Uint8Array) => {
  try {
    if (isPNG(logoBytes)) {
...
export const downloadInvoicePDF = async (party: any) => {
  try {
    ...
    // Convert the imported logo to a URL that can be fetched
    let logoUrl: string;
    if (typeof Logo === 'string') {
      logoUrl = Logo;
    } else {
      // For imported images, create a blob URL
